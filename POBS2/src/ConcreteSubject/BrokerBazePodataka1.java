...
public class BrokerBazePodataka1 extends BrokerBazePodataka // ConcreteSubject
{
   Connection conn = null;
   Subject sub;
   GeneralDObject odoObserver;
   
   public BrokerBazePodataka1(Subject sub1) {sub = sub1;}
   ...
   @Override
    ...
    {   Statement st=null;
  	...
  	try {   st  = conn.prepareStatement(upit);
                ...
                if (rowcount > 0) 
                  { signal = true; odoObserver = odo;}//sub.Notify(odo);}    
	    } catch (SQLException ex)  
                {   Logger.getLogger(BrokerBazePodataka1.class.getName()).log(Level.SEVERE, null, ex);
                    signal = false;
